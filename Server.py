...
class Cfg:
    ...
    def save(self) -> None:
        ...
        try:
            f = open('config', 'wb')
            bs = bytearray(struct.pack('6sx???h', b'config',
                           self.nopwd, self.log, self.auto_sleep, self.port))
...
def Help() -> str:
    ...
    if socket_server:
        ...
        s += 'Listening ' + C.address + '    IP: ' + ip_address+'\n'
        s += 'Runtime:\n'
        for i in HP_RUN:
            s += '  %8s  %s\n' % (i, HP_RUN[i])
    else:
...
Head = {"Host": "opqnext.com", "Pragma": "no-cache", "Referer": "http://opqnext.com/",
...
class WAV:
    def __init__(self) -> None:
        self.file = BytesIO()
        self.audio = PyAudio()
        self.rec = self.audio.open(
            format=16, channels=1, rate=20000, input=True, frames_per_buffer=1024)
    ...
    def play(self, dat=b'') -> None:
        ...
        pf.seek(0)  # 位置回到开头
        w = wave.open(pf, 'rb')
        p = self.audio.open(format=self.audio.get_format_from_width(
            w.getsampwidth()), channels=w.getnchannels(), rate=w.getframerate(), output=True)
...
UnknownErr = pack(res="HTTP/1.1 400 Bad Request", header=CTjson,
                  data=dumps({"code": 400, "message": "Unknown Error"}))
pwdErr = pack(res="HTTP/1.1 403 Forbidden", header=CTjson,
              data=dumps({"code": 403, "message": "Wrong password"}))
...
faviSame = pack(res='HTTP/1.1 304 Not Modified', header={
                'Content-Type': 'image/svg+xml', 'Cache-Control': 'public, max-age=31536000', 'ETag': etgFavi})
index = pack(res='HTTP/1.1 200 OK', header={"Content-Type": "text/html; charset=utf-8", 'Cache-Control': 'public, max-age=31536000'},
             data="""<!DOCTYPE html><html><head><meta charset="UTF-8"><meta http-equiv="refresh" content="0; url=/index.html"></head><body></body></html>""")
successResp = pack(res='HTTP/1.1 200 OK', header=CTjson,
                   data=dumps({"code": 200, "message": "success"}))
...
def getFile(loc, head=False, rang='', pETag='', token=0) -> bytes:
    ...
    if loc == "":
        ...
        dir = Diskdir
        resp = pack(res='HTTP/1.1 200 OK',
                    header={'Content-Type': 'application/json'}, data=dumps(dir))
        return resp
    ...
    if path.isdir(loc):
        ...
        try:
            ...
            for i in filelist:
                try:
                    ...
                    next
                f = {'name': i.name, 'mtime': s.st_mtime, 'mts': strftime(
                    '%Y/%m/%d %H:%M', localtime(s.st_mtime))[2:]}
                if i.is_file():
                    f['size'] = s.st_size
            ...
            resp = pack(res="HTTP/1.1 400 Bad Request", header=CTjson,
                        data=dumps({"code": 400, "message": str(e), "trace": fexc()}))
        ...
        header = {'Content-Length': size, 'Content-Type': guess_type(loc)[0], 'Accept-Ranges': 'bytes', 'Last-Modified': strftime(
            '%a, %d %b %Y %H:%M:%S GMT', localtime(curMod)), "Cache-Control": "public, max-age=31536000", 'ETag': ETag}
        if pETag == ETag:
            ...
            if rang:  # 只接受一组值，多组的特喵的自己再请求一次...别改这里，下大文件很容易出错
                ...
                if rang[0] == '':
                    ...
                    rang[1] = size-1 if (not cache) or size - \
                        rang[0] < maxs else maxs
                else:
                    rang[0] = int(rang[0])
                    rang[1] = int(rang[1])
                ...
                if rang[0] < 0 or rang[1] < rang[0] or rang[1] > size:
                    resp = pack(
                        res='HTTP/1.1 416 Range Not Satisfiable', data='')
            else:
                rang = [0, size-1, size]
            if rang[2] > Lim:
                rang[1] = rang[0]+Lim-1
            ...
            try:
                ...
                if cache:
                    if not Files.get(loc):
                        ...
                        log('\033[1;33m%-7s\033[0m%-21s %s' % ('GET',
                            (rg if rg else 'whole')+('  HEAD' if head else ''), loc))
                    else:
                        file = Files[loc]['file']
                        Files[loc]['last'] = time()
                else:
        ...
        log('\033[1;33m%-7s\033[0m%-21s %s' % ('GET',
            (rg if rg else 'whole')+('  HEAD' if head else ''), loc+" Not Found"))
    return resp
    ...
    while client.open:
            ...
            if dat == b'\x03\xe9':
                ...
                try:
                    if client.ws == ':remote' and Win:
                        ...
                        f = client.file.read()
                        client.socket.send(
                            packMes(struct.pack("h" * len(f), *f)))
                        # 传递录制帧
                        pass
                except Exception as e:
                    client.socket.send(
                        packMes(dumps({'message': 'Error: '+fexc()})))
            else:
        ...
        if head['Method'].upper() == "GET":
            if head.get('Upgrade') == 'websocket':
                ...
                resp = getFile(head['path'], rang=head.get('Range'), pETag=head.get(
                    'If-None-Match'), token=head.get('token'))
            ...
            if pt == ':host':
                ...
                if oprt == 'cmd':
                    if data.get('cmd'):
                        ...
                        cmds = cmd+' '
                        cmds = cmd if len(
                            cmd) <= 50 else cmds[:cmds.rfind(' ', 0, 60)]
                        if data.get('return'):
                            ...
                            try:
                                process = Popen(cmd, shell=True, bufsize=1, stdin=PIPE, stdout=PIPE,
                                                stderr=PIPE, universal_newlines=True, encoding='utf-8')
                                Pipes.append(process)
                                newkill(process, client, tm)
                                # 去除退格\x08和ESC控制符
                                output = sub(
                                    r'\x08|(\x1B\[\??\d*(;\d+)*[mA-Ksulh])', '', process.stdout.readline()).encode()
                                # 要用16进制表示每个节的大小
                            ...
                            try:
                                proc = Popen(
                                    cmd, shell=True, bufsize=0, stdout=PIPE, stderr=PIPE, encoding='utf-8')
                                Pipes.append(proc)
                                # 默认等待2s，如果结束了就返回结果
                                sleep(2)
                                dat = ''
                                ...
                                if dat == '':
                                    dat = 'start '+cmd
                                resp = pack(
                                    res='HTTP/1.1 200 OK', header={"Content-Type": "text/plain; charset=utf-8"}, data=dat)
                            except Exception as e:
                                msg = str(e)
                                trc = fexc()
                    else:
                    ...
                    if ip.qpwd > 3:
                        ...
                        resp = pack(res='HTTP/1.1 200 OK', header=CTjson,
                                    data=dumps({'code': 200, 'token': tok}))
                    else:
                    ...
                    if data.get('path'):
                        ...
                        try:
                            l = getLrc(data['url'])
                            resp = pack(
                                res='HTTP/1.1 200 OK', header={"Content-Type": "text/plain; charset=utf-8"}, data=l)
                        except Exception as e:
                            msg = str(e)
                            trc = fexc()
                    else:
        ...
        try:
            if resp:
                ...
                resp = pack(res="HTTP/1.1 400 Bad Request", header=CTjson,
                            data=dumps({"code": 400, "message": msg}))
                client.socket.send(resp)
        except:
            client.close()
    if client.open:
        try:
            if resp:
                ...
                resp = pack(res="HTTP/1.1 400 Bad Request", header=CTjson,
                            data=dumps({"code": 400, "message": msg, "trace": trc}))
                client.socket.send(resp)
        except:
            pass
        client.close()
...
def Manage(cmd):
    ...
    if cmd == '-c':
        ...
        if l == 0:
            ...
            s = ''
            for i in range(l):
                s += str(i+1) + ' running' if Pipes[i].poll(
                ) != None else ' closed' + Pipes[i].args+'\n'
            return s
    elif cmd == '-r':
        l = len(Pipes)
        n = 0
...
def init():
    ...
    if Win:
        ShowWindow(TM, 0)
    C.address = f'{C.address[0]}:{C.address[1]}'
    hostname = gethostname()
    ip_address = gethostbyname(hostname)
    ...
    log('\033[33mServer \033[1;33m%-21s \033[0mstarted at \033[33m%s\033[0m' %
        (C.address, Local))
    if Win:
        system('title Server '+C.address+' '+Local)
    # 线程1：监听连接
