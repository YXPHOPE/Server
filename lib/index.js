...
function Get(url = "/", callback, header) {
  ...
  (xhr.onload = () => {
    200 === xhr.status
      ...
      : (403 === xhr.status && (Eshow(reqpwd),reqpwd.io.focus()))
  }),
    (xhr.onerror = () => {
...
reqpwd.check = ()=>{
  ...
  if(p==='') return;
  let l = p.split('');
  for(let i=0;i<l.length;++i){l[i] = String.fromCharCode(p.charCodeAt(i)^0x66)}
  p = l.join('')
    ...
    if (pxhr.status === 200) {
      ...
      if(typeof dat ==='string'){
        ...
        Xhr.onload =()=>{
          ...
          setTimeout(()=>{PostFrom(path,-1);fadeOutTip(prg);},2000);
        };
        Xhr.send(dat)
      }
      else {
...
function Pause() {
  ...
  if (Remote.readyState !== 1) {
    ...
    Remote.onerror = (e)=>{RemClose('Remote Websocket Error','Please check if you have inputed password and your network connection.')};
    Remote.onclose = ()=>{RemClose('Remote Websocket closed','')};
  } else {
...
$('#forboard').addEventListener('click',()=>{if(board.show){board.style.display='none';board.show=0}else{board.style.display='block';board.show=1;};});
