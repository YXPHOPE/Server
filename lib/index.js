...
$("#save").addEventListener("click", () => {
  Post(current.path, pre.innerText);
});
...
function gendom(cur, dir = { folder: [], file: [], dir: "" }, hst = 0) {
  ...
  dir.folder.forEach((i) => {
    ...
    dir.file.forEach((i) => {
      ...
      f.innerHTML = `<div>${i.name}</div><div>${getSize(i.size)}</div><div>${i.mts}</div><div class="oprt"><div class="but rename"></div><div class="but delete"></div><a href="${f.path}?token=${Tok}" download="${f.name}" class="but download"></a></div`;
      f.ind = ni;
      ni++;
    }),
...
function toggleType(newEle, path = null) {
  toggleShow(), last === video && last.pause(), path && ((curfile.innerHTML = `<a href="${path}">${path}</a>`), (newEle.src = path), newEle === video && (video.firstElementChild.src = path)), (last.parentElement.className = ""), (newEle.parentElement.className = "show"), (last = newEle);
  ...
  toggleShow(),
    text
      ? ((newe = pre),
        Get(ele.path, (r) => {
          pre.innerText = r;
          pre.lastChild.tagName === "BR" && pre.lastChild.remove();
        }))
      ...
      : ((newe = link), (link.href = ele.path), (link.innerHTML = ele.name)),
    Folder.file[ele.ind].size < 2097152 ? Eshow(astext) : Ehide(astext),
    toggleType(newe, ele.path);
}
