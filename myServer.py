...
# 命令行：color、title
from os import getcwd, system, rename, remove, path, chdir, makedirs, scandir
        ...
        if pETag == ETag:
            ...
            try:
                ...
                if rang:
                    ...
                    resp = pack(res='HTTP/1.1 206 Partial Content', header=header, data=c)
                else:
                    c = file.read()
                    resp = pack(res='HTTP/1.1 200 OK', header=header, data=c)
                if not cache:
                    file.close()
            except Exception as e:
                resp = pack(res="HTTP/1.1 400 Bad Request", header=CTjson, data=dumps({"code": 400, "message": str(e)}))
...
def maintain():
    while True:
        ...
        w = []
        for i in Tokens:
            if now-Tokens[i]>7200:w.append(i)
        for i in w:
            Tokens.pop(i)
def Manage(cmd):
    f = ''
